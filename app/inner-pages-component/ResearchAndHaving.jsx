...
const ResearchAndHaving = ({title, description, subtitle, description2, rightContent, whyUseBtn, badeTitle, flexReverse = false}) => {
                    ...
                    <div className={`flex flex-col md:grid ${gridDirectionClass} gap-8 md:gap-12 lg:gap-20 items-center`}>
                        ...
                        {(hasContent(title) || hasContent(description) || hasContent(subtitle) || hasContent(description2)) && (
                            <div className={`content w-full ${flexReverse ? 'md:order-1' : 'md:order-2'}`}>
                                ...
                                {hasContent(title) && (
                                    <h2 data-aos="fade-up" data-aos-duration="500" className='text-2xl sm:text-3xl md:text-4xl lg:text-[35px] font-bold text-left leading-tight mb-3 md:mb-4'>
                                        {title.split(' ').map((word, index) => (
                                            <span key={index} className={index === 2 || index === 3? 'text-[#B4FE5D]': 'text-white'}>
                                                {word}
                                ...
                                {hasContent(subtitle) && (
                                    <h3 data-aos="fade-up" data-aos-duration="700" className='text-xl sm:text-2xl md:text-3xl lg:text-[35px] font-bold text-left leading-tight mb-3 md:mb-4'>
                                        {subtitle.split(' ').map((word, index) => (
                                            <span key={index} className={ index === 1 || index === 2 ? 'text-[#B4FE5D]' : 'text-white'}>
                                                {word}
                                ...
                                {hasContent(description2) && (
                                    <p data-aos="fade-up" data-aos-duration="600" className='text-left text-gray-300 text-sm sm:text-base md:text-lg leading-relaxed'>
                                        {description2}
                                    </p>
                                )}
                       ...
                       {rightContent && (
                            <div className={`flex items-center cursor-pointer h-full p-6 rounded-2xl border border-[#9EF01A]/20 hover:border-[#B4FE5D]/40 transition-all duration-300 bg-gradient-to-b from-transparent to-[#9EF01A]/9 hover:-translate-y-2 hover:shadow-lg hover:shadow-[#B4FE5D]/10 ${flexReverse ? 'md:order-2' : 'md:order-1'}`}>
                               <h3 className='text-white text-2xl sm:text-3xl md:text-[42px] lg:text-[45px] font-bold leading-tight'>{rightContent}</h3>
                            </div>
                        )}
                    </div>
                </div>
...
export default memo(ResearchAndHaving);